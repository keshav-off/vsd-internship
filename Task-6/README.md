...
The design was synthesized and programmed onto the VSDSquadron FPGA Mini board (iCE40UP5K) using the open-source toolchain.
...
**Clock source:** 12 MHz onboard crystal oscillator.
 
**Firmware used:** `Timer_periodic_test` with `LOAD = 12000000`
 
```
Tick period = 12000000 / 12000000 Hz = 1 second
```
 
**Observed behavior:** `LED0` toggles every 1 second in response to real `TIMEOUT` events generated by the Timer IP running on actual silicon. The toggling repeats continuously while the board is powered.
 
**What this validates on hardware:**
 
| Behavior | Result |
|----------|--------|
| Countdown at correct rate | LED toggles at exactly 1-second intervals |
| `timeout_o` signal assertion | Correctly drives LED toggle logic in SoC |
| Periodic auto-reload | `value_reg` reloads from `LOAD` after each timeout without software intervention |
| Register write path | CTRL and LOAD correctly programmed through CPU bus on real silicon |
 
**Video evidence:** `hardware_demo.mp4` in `/test` directory.
 

https://github.com/user-attachments/assets/6d683d33-6d8d-480c-a300-276dcb83395e
...
| Test | Firmware | LOAD Value | Mode | Simulation | Hardware |
|------|----------|------------|------|------------|----------|
| One-shot | `Timer_test.c` | 0x186A0 (100000) | One-shot | TIMEOUT set, VALUE counted down | — |
| Periodic | `Timer_periodic_test.c` | 0x14 (20) sim / 12000000 hw | Periodic | 3 reloads observed | LED0 toggles every 1 second |
| W1C clear | `Timer_clear_test.c` | 0x0A (10) | One-shot | TIMEOUT set, cleared to 0 | — |
| LED output | All three | — | All | LEDS[0] toggled in all sim logs | Confirmed on board |
